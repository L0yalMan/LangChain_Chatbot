...
export default function FileUpload() {
  ...
  const confirmDeleteWebsite = async () => {
    ...
    try {
      // Delete from S3
      const response = await axios.post(`https://f8c14eefce75.ngrok-free.app/files/delete/`, { filename: fileToDelete.name, userId: user?.id })
  ...
  const handleUpload = async () => {
    ...
    try {
      await Promise.all(selectedFiles.map(async (selectedFile) => {
        ...
        try {
          const response = await axios.post("https://f8c14eefce75.ngrok-free.app/files/upload", formData)
          if(response.status === 200) {
